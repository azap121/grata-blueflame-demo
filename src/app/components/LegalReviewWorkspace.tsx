        ...
        >
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ width: '9%' }}>Deal team notes</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {QA_TRIAGE_ITEMS.map((row) => {
              const firstCitationId = row.citationFileIds[0];
              const firstCitation = STIFEL_SOURCE_FILES.find((file) => file.id === firstCitationId);
              return (
      ...
      >
        <Stack direction="row" spacing={0.75} alignItems="center" sx={{ color: 'text.secondary', minWidth: 0 }}>
          ...
          <Typography sx={{ fontSize: 12.5 }}>
            Suggested answers stay draft-only until you route, review, or mark them ready for approval.
          </Typography>
        </Stack>
