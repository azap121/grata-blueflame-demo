...
export default function ValidationPlanWorkspace({
  ...
  const isBrief = state.flow === 'brief';
        ...
        <Stack spacing={0.75} sx={{ minWidth: 0, flex: 1 }}>
          <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
            <Typography sx={{ fontSize: 18, fontWeight: 650, color: 'text.primary' }}>
              {isBrief ? 'State-of-the-room brief plan' : 'Q&A triage plan'}
            </Typography>
            <Chip
              size="small"
      ...
      >
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', md: 'center' }} justifyContent="space-between">
          <Typography sx={{ fontSize: 12.5, color: 'text.secondary', lineHeight: 1.5 }}>
            {isBrief ? 'The brief is drafted only after you approve this plan.' : 'The Q&A run starts only after you approve this plan.'}
          </Typography>
          <Stack direction="row" spacing={1} justifyContent="flex-end">
            ...
            <HaloButton
              ...
              sx={{ textTransform: 'none', whiteSpace: 'nowrap' }}
            >
              {approved ? (isBrief ? BRIEF_COPY.approvedPlanMessage : COPY.approvedPlanMessage) : isBrief ? BRIEF_COPY.approvePlanCta : 'Approve and triage'}
            </HaloButton>
          </Stack>
        </Stack>
      </Box>
