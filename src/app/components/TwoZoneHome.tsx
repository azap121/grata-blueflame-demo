...
interface Props {
  ...
  // 'home' renders both zones; 'projects' renders only the projects grid.
  variant?: 'home' | 'projects';
  onOpenDeal: (deal: DealCard) => void;
  onStartSourcing: (query: string) => void;
  onAsk: (prompt: string) => void;
  onOpenRuns: () => void;
  // Needs-you CTA deep-links: opens the named project.
  onOpenProjectByName: (projectName: string) => void;
}

export default function TwoZoneHome({
  deals,
  freshDealId,
  variant = 'home',
  onOpenDeal,
  ...
  return (
    <Box sx={{ height: '100%', minHeight: 0, overflowY: 'auto', bgcolor: 'background.paper' }}>
      <Box sx={{ maxWidth: 1080, mx: 'auto', px: { xs: 3, md: 6 }, py: { xs: 3, md: 4.5 } }}>
        {/* ── Zone 1 · the intelligence composer ─────────────────────────── */}
        <Stack spacing={1.5} sx={{ maxWidth: 760, mx: 'auto', mb: 4 }}>
          <Typography
            component="h1"
            sx={{ fontSize: 30, fontWeight: 300, letterSpacing: '-0.02em', color: 'text.primary', textAlign: 'center' }}
          >
            {HOME_HEADLINE}
          </Typography>
          <Paper
        ...
        <Stack spacing={3.5}>
          <Box>
            <SectionEyebrow label="Needs you" count={NEEDS_YOU.length} />
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 1.5 }}>
              {NEEDS_YOU.map((item) => (
                <NeedsYouCard key={item.id} item={item} onOpen={() => onOpenProjectByName(item.project)} />
              ))}
            </Box>
          </Box>
          ...
          <Box>
            <SectionEyebrow
              label="Overnight"
              action={
                <HaloButton size="small" variant="text" onClick={onOpenRuns} sx={{ textTransform: 'none' }}>
                  View all runs →
                </HaloButton>
              }
            />
...
// Always-visible scope control — defaults from context (Universe on home).
...
function SectionEyebrow({ label, count, action }: { label: string; count?: number; action?: React.ReactNode }) {
  return (
...
const NEEDS_YOU_ICONS = {
  gate: faCircleCheck,
  qa: faCommentsQuestion,
  review: faTableCells,
} as const;

function NeedsYouCard({ item, onOpen }: { item: NeedsYouItem; onOpen: () => void }) {
  return (
    <Box
      component="button"
      type="button"
...
function OvernightRow({ item }: { item: OvernightItem }) {
  const icon = item.kind === 'signal' ? faBolt : faCheck;
  return (
    <Stack direction="row" alignItems="center" spacing={1.25} sx={{ px: 1.75, py: 1.1 }}>
      <FontAwesomeIcon icon={icon} style={{ fontSize: 12, color: moondust[500] }} />
      <Typography sx={{ fontSize: 13, color: 'text.primary', flex: 1 }}>{item.title}</Typography>
      <Typography sx={{ fontFamily: monoFontFamily, fontSize: 11, color: 'text.secondary' }}>{item.project}</Typography>
      <Typography sx={{ fontFamily: monoFontFamily, fontSize: 11, color: 'text.disabled' }}>{item.timestamp}</Typography>
    </Stack>
  );
}
