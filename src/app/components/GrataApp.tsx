...
// Citation sources must resolve before the right canvas ever mounts (its own effect
...
export default function GrataApp() {
  ...
  const reviewOpen = openRightCanvasTabs.includes('enhanced-index') || openRightCanvasTabs.includes('filing-review');
  ...
  const dispatch = useCallback((action: WorkspaceAction) => {
    ...
    const createsOrUpdatesThread = action.type === 'SELECT_FOLDER_PROMPT' || action.type === 'SELECT_FILING_PROMPT' || action.type === 'SELECT_BRIEF_PROMPT' || action.type === 'CHAT_PROMPT_SUBMITTED';
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [activeCoreTab, dispatch, state.flow, state.planStepIndex, state.stage]);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [activeCoreTab, dispatch, state.flow, state.recommendationStepIndex, state.stage]);

  useEffect(() => {
    if (activeCoreTab !== 'ai') return undefined;
    if (state.stage !== 'save-processing') return undefined;
    const saveStepCount = state.flow === 'filing' ? filingSaveSteps.length : saveSteps.length;
  ...
  const handleOpenFilingReview = useCallback(() => {
    ...
    updateActiveSession((session) => ({
      ...
      rightCanvasOpen: true,
      rightCanvasDisplayMode: 'default',
      openRightCanvasTabs: session.openRightCanvasTabs.includes('filing-review')
        ? session.openRightCanvasTabs
        : [...session.openRightCanvasTabs, 'filing-review'],
      activeRightCanvasTab: 'filing-review',
    }));
    setRightCanvasMotion(reducedMotion ? 'idle' : 'entering-from-right');
    if (!reducedMotion) {
      setReviewTransitionTimer(() => setRightCanvasMotion('idle'), REVIEW_ENTER_MS);
    }
  }, [clearReviewTransitionTimers, dispatch, setReviewTransitionTimer, updateActiveSession]);

  const handleOpenCitation = useCallback((fileId: string) => {
    clearReviewTransitionTimers();
    const tab = `file:${fileId}` as RightCanvasTab;
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    updateActiveSession((session) => ({
      ...session,
      rightCanvasOpen: true,
      ...
      <Box sx={{ height: '100%', minHeight: 0, overflow: 'auto', borderRadius: '16px', bgcolor: 'background.paper' }}>
        <Stack spacing={3} sx={{ width: 'min(880px, 100%)', px: { xs: 3, md: 6 }, py: 5 }}>
          ...
          <NotesCard
            ...
            body="Does the deal team type a single keyword such as churn, or naturally ask a richer question like finding support for churn and NRR claims that can be shared with Round 1 buyers? Watch whether Tom and Jaime differ."
          />
          <NotesCard
...
function createFolderOverviewCopy(folder: SellerIndexFolder, files: SellerIndexFile[]) {
  ...
  return [
    `I reviewed ${folderPath}. It contains ${fileCount} uploaded item${fileCount === 1 ? '' : 's'}${fileTypes ? ` across ${fileTypes}` : ''}. For the Q&A workflow, I would treat this as answer support: the value is in checking whether the files are cited, permissioned correctly, and safe to reference.`,
    currentEvidence > 0
    ...
    evidenceSignals.length > 0
      ? `The points I would check first are ${evidenceSignals.join('; ')}. If you want to answer a buyer, I would keep the underlying files cited and frame restricted content as review-required.`
