...
- **CIM run** — `state/cimRunScenario.ts` phase machine (`idle → working → plan-ready → executing → output-ready → accepted`), keyed off the playbook id via `QUEUE_PLAYBOOK` (Agent card click stages the prompt + arms the engine; submit never parses composer strings). Timers live in `GrataApp` effects, AX cadence 950/1050ms, reduced-motion collapses to 80ms.
